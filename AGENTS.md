...
### Run Linter
The repository enforces linting with `golangci-lint`:
```bash
...
1. **Verification**: Always run `go test ./...` and `go build ./...` after introducing changes to verify compilation and test suites.
